...
const FINAL_SIMULATION_CSV_PATH = 'data/final_simulation_mean_results.csv';
...
function buildStickyVisualizationPanel() {
    ...
    const panelHtml =
        ...
        '<svg id="viz-chart" viewBox="0 0 ' + VIZ_CHART.width + ' ' + VIZ_CHART.height + '" preserveAspectRatio="xMidYMid meet"></svg>' +
...
async function loadSimulationSeries() {
    const response = await fetch(FINAL_SIMULATION_CSV_PATH, { cache: 'no-store' });
    if (!response.ok) {
        throw new Error('HTTP ' + response.status + ' for ' + FINAL_SIMULATION_CSV_PATH);
    }
...
function populateModelStatsCards() {
    ...
    frames.forEach(function (frameData, index) {
        ...
        const selector = '.frame[data-frame-index="' + index + '"] .frame-card-text';
...
function buildModelStatsMarkup(row, targetYear) {
    ...
    return (
        ...
        '<li>Black/White wealth ratio (actual): <strong>' + formatPercent(actualRatio) + '</strong></li>' +
        '<li>Black/White wealth ratio (40 acres scenario): <strong>' + formatPercent(scenarioRatio) + '</strong></li>' +
        '<li>Scenario vs actual Black wealth uplift: <strong>' + formatPercent(blackWealth > 0 ? (black40Wealth - blackWealth) / blackWealth : 0) + '</strong></li>' +
        '</ul>'
    );
}
...
function buildStatsGroup(label, data) {
    return (
        ...
        '<h4>' + label + '</h4>' +
        '<ul>' +
        '<li>Mean wealth: <strong>' + formatMoney(data.wealth) + '</strong></li>' +
        '<li>Mean land value: <strong>' + formatMoney(data.land) + '</strong></li>' +
        '<li>Mean investments: <strong>' + formatMoney(data.invest) + '</strong></li>' +
        '<li>Mean income: <strong>' + formatMoney(data.income) + '</strong></li>' +
        '</ul>' +
        '</section>'
    );
}
...
function getTargetYearForFrame(frameIndex) {
    ...
    const firstYear = simulationSeries[0].year;
    const lastYear = simulationSeries[simulationSeries.length - 1].year;
    const clampedFrameIndex = Math.max(plotStartFrameIndex, frameIndex);
    const vizStepIndex = visualizationFrameIndices.indexOf(clampedFrameIndex);
...
function renderLineChart(targetYear) {
    ...
    yTicks.forEach(function (tickValue) {
        ...
        chartMarkup +=
            ...
            '<text class="viz-axis-label y-axis-label" x="' + (padding.left - 12) + '" y="' + y + '">' + formatYAxisTick(tickValue) + '</text>';
    });
    ...
    VIZ_SERIES.forEach(function (series) {
        ...
        if (!linePath) return;
        chartMarkup += '<path class="viz-series-line" d="' + linePath + '" stroke="' + series.color + '"></path>';
    });
    ...
    VIZ_SERIES.forEach(function (series) {
        ...
        chartMarkup += '<circle class="viz-series-point" cx="' + x + '" cy="' + y + '" r="3.5" fill="' + series.color + '"></circle>';
    });

    svg.innerHTML = chartMarkup;

    $('#viz-year-readout').text('Visible data through ' + targetYear + ' (log scale, from final_simulation_mean_results.csv)');
}
